...
# Custom ZAP working directory outside of /home to prevent labtainer artifact bloating
...
if [ "$OS" = "Darwin" ]; then
  ...
  exec java ${JMEM} ${JAVAGC} -Xdock:icon="../Resources/ZAP.icns" -jar "${BASEDIR}/zap-2.9.0.jar" -dir "$ZAP_HOME" "${ARGS[@]}"
else
  exec java ${JMEM} ${JAVAGC} ${JAVADEBUG} -jar "${BASEDIR}/zap-2.9.0.jar" -dir "$ZAP_HOME" "${ARGS[@]}"
fi
